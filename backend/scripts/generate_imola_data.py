...
def build_imola_dataset(
    num_points: int = 2000, circuit_name: str = "Imola"
) -> Dict[str, np.ndarray]:
    """
    Build a telemetry dataset indexed by lap distance,
    with realistic signal values for Imola circuit.

    Args:
        num_points: number of sampled points along the lap
        circuit_name: track name (e.g., "Imola")

    Returns:
        Dictionary with lap_distance and signal variables
    """
    lap_distance = np.linspace(0.0, 4909.0, num_points)  # Imola ~4.9 km
    distance_step_m = np.array([float(np.median(np.diff(lap_distance)))])

    # Imola is a street circuit with long straights and technical corners
...
def main() -> None:
    """Generate Imola demo dataset and save to data/ directory."""
    repo_root = Path(__file__).resolve().parents[2]
    data_dir = repo_root / "data"
    data_dir.mkdir(parents=True, exist_ok=True)

    print("Generating Imola telemetry dataset...")

    # Build dataset
    dataset = build_imola_dataset(num_points=5000, circuit_name="Imola")

    # Save .mat file
    mat_path = data_dir / "imola.mat"
    savemat(str(mat_path), dataset)
    print(f"✓ Saved {mat_path}")

    # Save track layout
    track_path = data_dir / "imola_track.csv"
    build_track_csv(track_path, dataset["lap_distance"], circuit_name="Imola")
